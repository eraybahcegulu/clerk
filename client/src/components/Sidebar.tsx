...
const Sidebar = () => {
    ...
    return (
        <div className={`transition-all overflow-auto bg-slate-300 shadow-2xl flex flex-col p-2 ${isOpen ? 'w-72' : 'w-24'}`}>
            ...
            <div className='flex flex-col gap-4 p-4'>
                {navItems.map((item) => (
                    <Link
                        ...
                        to={item.href}
                        className={`flex group flex-row gap-3 items-center p-2 w-full cursor-pointer rounded-xl ${location.pathname === item.href ? 'bg-slate-400' : 'bg-white hover:bg-slate-100'
                            }`}
                    >
                        ...
                        <div className={`absolute z-10 ml-[40px] hidden ${!isOpen && 'group-hover:flex'} `}>
                            <div className='bg-slate-500 text-white relative flex items-center p-2 rounded-[3px]'>
